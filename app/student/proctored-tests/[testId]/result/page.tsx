...
const BANNER: Record<string, { icon: typeof CheckCircle2; title: string; tone: string; message: string }> = {
  submitted: {
    icon: CheckCircle2,
...
export default function ProctoredTestResultPage() {
  ...
  const banner = BANNER[status] ?? BANNER.submitted;
  ...
  return (
    <div className="mx-auto flex max-w-md flex-col items-center gap-4 py-16 text-center">
      ...
      <Link href="/student/proctored-tests" className="btn-secondary mt-4">
        Back to Proctored Tests
      </Link>
    </div>
  );
}
