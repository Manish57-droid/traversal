...
| **"Protocol test" / simulated placement drive** (multi-round, timed, sequential, pass/fail gated) | **IN PROGRESS** | **Proctored Tests** now has a full end-to-end MCQ exam flow, built in two slices. Schema/authoring (first slice): a class-scoped exam type with its own MCQ-only question bank (`proctored_questions`, not shared with Aptitude/DSA), test authoring (`proctored_tests` + ordered `proctored_test_questions`), class-gated via `getClassAuthorization` — teacher/admin UI at `/teacher/proctored-questions` (bank CRUD) and a "Proctored tests" panel on `/teacher/dashboard`. The bank itself is now organized as **Subject -> Set -> Question** (`proctored_subjects`/`proctored_sets`, shared/unscoped like DSA/Aptitude — not the class-scoped `proctored_tests` above), with a "Bulk Add from Paste" importer that parses freeform pasted MCQs through a mandatory review screen before anything saves, legacy questions flagged `needs_categorization` until assigned a Set, and a per-Set/Subject Excel export for a teacher's own records. **A test is now multi-section** (e.g. "TCS Preparation" with a Quantitative section and a Technical section, each on its own Subject) — the creation form builds one or more sections, each with a Subject and toggled-on Sets (a section's questions are resolved dynamically from those Sets' current contents, not frozen at creation time), an optional per-section time limit and negative-marking override, and a calculator toggle; a test picks `timer_mode` (combined vs. per-section) and whether free navigation between sections is allowed. Every test made before this restructuring still works unchanged — migration `0013` wrapped each one's existing questions into a single auto-created default section, verified against the real data (including a test with zero questions, and two 20-question tests that kept their exact original ordering). Scoring now sums across sections, each using its own negative-marking override when set. **The take screen is now fully section-aware**: `/student/proctored-tests/[testId]/take` shows a NEET-style tab row per section, a **6-state color-coded question palette** (current/answered/not-answered/not-visited/marked-for-review/answered-and-marked, driven by a persisted `question_status` jsonb column — `visited`/`marked_for_review` per question, `answered` stays derived from `answers`), three answer controls (Save & Next, Clear Response, Mark for Review & Next — each autosaves immediately, same principle as before), a header timer that shows the whole-attempt countdown in `combined` mode or the *current section's own* countdown in `per_section` mode (a section's clock starts only on first entry, server-authoritative via `proctored_section_attempts.started_at`), automatic section-submit-and-advance when a section's timer expires, hard section-lock enforcement (client AND server-side) when `allow_free_section_navigation` is off, and a toggleable on-screen calculator shown only for sections with `calculator_enabled`. `/student/proctored-tests/[testId]/start` (the pre-test rules screen) now lists per-section rules — resolved negative marking and calculator availability, stated once if every section shares the value or broken out per-section where they differ — alongside the existing camera/mic check and violation rules. `/student/proctored-tests/[testId]/take` (fullscreen exam UI), server-side violation logging + auto-submit at threshold, teacher-controlled result release gating a full review page. **Leaderboards** are now part of the module: `lib/leaderboard.ts` ranks a test's finished attempts (score DESC, then earlier `submitted_at` wins ties — a real SQL `ORDER BY`, not an app-code sort), shared by three surfaces — the student dashboard's "My Proctored Tests" history table (rank/score only ever sent once a test's `results_released` is true, same gating discipline as the full review endpoint) and its "Top 5" widget for the student's most recently completed released test, plus the teacher's per-test results view, which shows the full ranked leaderboard **unconditionally** (a teacher always sees it, release status only ever gates what a *student* sees). **Aptitude Test Mode does not have leaderboards yet** — this was built for Proctored Tests only; extending the same `lib/leaderboard.ts`-style approach to Aptitude is a plausible future task, not started. **Camera-vision checks** (`components/proctored-take/useCameraProctoring.ts`) run on top of the existing camera-on check when `require_camera` is set: MediaPipe Tasks Vision (WASM runtime loaded from jsDelivr, ML model weights fetched from Google's public MediaPipe model bucket) analyzes the live video element entirely client-side, on a ~2.5s tick, for three new logged violation types — `phone_detected` (general-purpose object detector, not phone-specific), `multiple_people` (a second distinct face, not a body-shaped blob — more reliable, still needs both faces lit and roughly camera-facing), and `talking_detected` (the `jawOpen` blendshape sustained across several samples — indistinguishable from chewing or a long yawn, there's no audio involved at all) — plus a fourth, `looking_away` (head yaw estimated from face-landmark positions), which is **deliberately a toast warning, never a logged violation**, since it's the least precise of the four. **Honest limits, read this before assuming more than it does**: (1) camera/mic, when required, is a **live browser check only** — frames (including the new vision checks above) are analyzed and discarded in-browser, never recorded, saved, or uploaded anywhere, so there is no video evidence to review after the fact, only the violation log, and every vision-based check is a heuristic with real false-positive/false-negative rates, not a certainty; (2) nothing here can detect or block another running application, a second monitor, or remote-desktop software — a browser has no access to that; this is **detection-and-response inside the tab** (fullscreen-exit, tab-switch/blur, copy/cut, camera dropping out, plus the camera-vision checks above), not prevention of every way to cheat; (3) every security-relevant decision (violation count, elapsed time, final score) is computed and enforced server-side — the client only ever reflects state, confirmed by directly rewinding an attempt's `started_at` in the database and observing the server independently close it out as expired, ignoring whatever the client's own countdown displayed. Still not built: multi-round drives (this is single-test, not the full sequential-rounds "drive" concept from Section 5's data model). |
...
- **proctored_subjects** / **proctored_sets** — the bank's organization: a Subject holds any
  number of Sets (`subject_id` FK, cascade). Shared bank, any teacher/admin can create/edit — not
  class-scoped, same spirit as the DSA/Aptitude banks.
- **proctored_questions** — an MCQ for the Proctored Tests bank (separate from Aptitude/DSA):
  ...
  `set_id` (nullable FK to `proctored_sets`), `needs_categorization` (flags legacy/unassigned
  questions predating the Subject -> Set structure — same pattern as DSA's `needs_link_curation`).
- **proctored_tests** — a class-scoped timed exam: `class_id` (NOT NULL — belongs to exactly one
  class), `name`, `description`, `time_limit_minutes` (the combined/fallback timer),
  `negative_marking_fraction` (the test-level default), `max_violations_before_autosubmit`,
  `require_camera`, `require_mic`, `results_released` (teacher-controlled gate on the full
  per-question review page), `timer_mode` ('combined'/'per_section'),
  `allow_free_section_navigation`. Always has **one or more sections** — a test is never a flat
  question list.
- **proctored_test_sections** — a test's sections (e.g. "Quantitative", "Technical"): `name`,
  `subject_id` (nullable — null only for a legacy default section, see below), `position`,
  `time_limit_minutes`/`negative_marking_fraction` (both nullable — null means "inherit the
  test-level value"), `calculator_enabled`.
- **proctored_test_section_sets** — which Sets are enabled for a section (`section_id`, `set_id`).
  A section's actual questions are resolved **dynamically** from this at read time (every question
  whose `set_id` matches an enabled Set) — not snapshotted, so editing a Set's contents after test
  creation is reflected on a student's next attempt. `lib/proctoredSections.ts` is the one place
  this resolution happens, shared by scoring, attempt-start, review, and the Excel report.
- **proctored_test_questions** — kept as a **legacy-only** path: every test created before the
  multi-section restructuring had its existing question rows wrapped into one auto-created default
  section by migration `0013` (`section_id` column, `subject_id: null` since those questions
  predate Subject/Set and may span several or none); a section created through the new flow never
  writes rows here. Ordered by `position` within its `section_id`.
- **proctored_section_attempts** — one row per (attempt, section): `started_at`, `submitted_at`,
  `status` (not_started/in_progress/completed) — created for every section up front when an
  attempt starts, regardless of navigation mode. Drives the take screen's per-section timer and
  section-lock enforcement (both client- and server-side — the `sections/[sectionId]` endpoint
  rejects an out-of-order or already-completed section regardless of what the client does).
- **proctored_test_attempts** — one row per (student, test): `status`
  (in_progress/submitted/auto_submitted_violation/expired), `answers` (jsonb), `question_status`
  (jsonb — `{visited, marked_for_review}` per question; `answered` stays derived from `answers`;
  together these three booleans plus "currently on screen" drive the take screen's 6-state
  palette), `score` (now summed across every section, each scored with its own negative-marking
  override when set, else the
  test-level value), `total_questions`, `violation_count`, `time_taken_seconds`, timestamps.
- **proctored_violations** — full audit log per attempt (`violation_type`: tab_switch/
  ...
  this log is what a teacher reviews instead. Violation threshold stays test-wide, not per-section.
